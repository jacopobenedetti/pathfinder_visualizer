...
export const gbfs: AlgorithmFn = (grid, start, target): AlgorithmRun => {
  ...
  const push = (coord: Coord) => {
    open.push(coord);
    open.sort((a, b) => {
      const ha = hScore[a.row][a.col];
      const hb = hScore[b.row][b.col];
      if (ha !== hb) return ha - hb;
      return gScore[a.row][a.col] - gScore[b.row][b.col];
    });
  };
  ...
  while (open.length > 0) {
    const current = open.shift()!; // sorted by heuristic, then g
